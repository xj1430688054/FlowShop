...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <li><a href="/FlowerShop/UserMessageCl?type=aftersearch" onClick="return CheckLogin(<%=user %>);">我要留言</a></li>
    ...
    <li><a href="/FlowerShop/UserClServlet?type=showcontact">联系方式</a></li>
  	...
  	<!--  <div class="check1">感谢您的光临，以下为您的所有订单</div>-->
  	<div class="user_info">所有订单信息<span style="float: right;margin-right: 10px;"><a href="/FlowerShop/index.jsp" style="text-decoration: none;color:#92E587;">返回首页</a></span></div>
  	...
  	<table border="0">
  	<tr><td>收花人</td><td>联系电话</td><td>付款方式</td><td>总价</td><td>交易状态</td><td>交易操作</td></tr>
  	</table>
  	</div>
  	
  	<%
  			if(allOrderInfo.size()==0){//还没有订单
  	%>
  				<div class="show_order_info1">
  						<span>您还没有预订过，谢谢!</span>
  				</div>
  	<% 
  	...
  	<%
  		for(int i=0;i<allOrderInfo.size();i++){
  			OrderInfo orderInfo = (OrderInfo)allOrderInfo.get(i);
  			//付款方式
  	...
  	%>
  		...
  		<div class="single_orderinfo"><span><%=orderInfo.getTruename() %></span></div>
  		<div class="single_orderinfo"><span><%=orderInfo.getPhone() %></span></div>
  		<div class="single_orderinfo"><span><%=orderInfo.getPayMode()%></span></div>
  		<div class="single_orderinfo"><span>￥<%=orderInfo.getTotalPrice() %></span></div>
  		<div class="single_orderinfo"><span style="color: red;"><%=orderInfo.getTradestate() %><br/><br/>
  		<%
  						if("网银付款".equals(paymode)&& !("交易关闭".equals(tradestate))&& ("否".equals(ispayed))&& !("交易成功".equals(tradestate))){	//去付款的前提是:①付款方式为网银付款;②交易未关闭;③订单未付款
  						%>
  		...
  		<div class="single_orderinfo"><span><input type="button" value="订单详情" onClick="getOrderDetail(<%=orderInfo.getId() %>)"/><br/><br/>
  		<%
  					if(!("交易关闭".equals(tradestate))&& !("交易成功".equals(tradestate))&&!("买家已付款".equals(tradestate))&& !("卖家已发货".equals(tradestate))){  //当订单未关闭，未成功，用户未付款时
  					%>
