...
FlowerTrendsList flowerTrendsList_before = (FlowerTrendsList)request.getAttribute("trends_before");
FlowerTrendsList flowerTrendsList_next = (FlowerTrendsList)request.getAttribute("trends_next");
...
<html>
  <head>
	...
	//二号线:发送http请求
	//使用Ajax异步修改订单
	function addtimes(flowernewsid){
			//window.alert("进来了=======");
			...
			if(myXmlHttpRequest){	//创建成功
				...
				//创建连接服务器的url
				var url="/FlowerShop/ShowFlowerTrendsCl?type=addtimes&mytime="+new Date();
				var data="id="+flowernewsid+"&mytime="+new Date();
				//打开请求
...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <li><a href="/FlowerShop/UserMessageCl?type=aftersearch" onClick="return CheckLogin(<%=user %>);">我要留言</a></li>
   ...
   <div class="content_right_top"><span>>>花卉动态</span></div>
   
   <!-- 右下部分-->
   <div class="show_flower_trends_info">
    <div class="trend_info_title"><%=flowerTrendsList.getTitle() %></div>
     <div class="trend_info_time"><span>发布时间: <%=flowerTrendsList.getNewstime() %></span> <span>浏览次数: <%=flowerTrendsList.getReadtimes() %></span></div>
      <div class="trend_info_cont"><%=flowerTrendsList.getContent() %></div>
      <%
      		if(flowerTrendsList_before.getTitle() !=null){
      	%>
      			<div class="trend_info_before">上一条: <a href="/FlowerShop/ShowFlowerTrendsCl?type=nr&no=<%=flowerTrendsList_before.getId() %>" onclick="addtimes(<%=flowerTrendsList_before.getId() %>)" ><%=flowerTrendsList_before.getTitle() %></a></div>
      	<% 
      		}
       %>
       <%
       		if(flowerTrendsList_next.getTitle()!= null){
       %>	
       			<div class="trend_info_next">下一条: <a href="/FlowerShop/ShowFlowerTrendsCl?type=nr&no=<%=flowerTrendsList_next.getId() %>" onclick="addtimes(<%=flowerTrendsList_next.getId() %>)" ><%=flowerTrendsList_next.getTitle() %></a></div>
       <% 
       		}
        %>
