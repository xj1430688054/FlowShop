   ...
   <%
    ...
    <% 
    			for(int m=0;m<al.size();m++){
    			   FlowerKnowledge flowerKnowledge = (FlowerKnowledge)al.get(m);
    %>			
    			<!-- 阅读次数的修改方法：每点击一次超链接，使用Ajax异步修改数据库中的阅读次数 set readtimes=readtimes+1 where id=? -->
    			<tr><td align="left"><a href="/FlowerShop/ShowFlowerKnowledgeCl?type=nr&no=<%=flowerKnowledge.getId() %>" onclick="addtimes(<%=flowerKnowledge.getId() %>)"><%=flowerKnowledge.getTitle() %></a></td><td align="right">阅读次数(<%=flowerKnowledge.getReadtimes() %>)  <%=flowerKnowledge.getPublishtime() %></td></tr>
    ...
    <% 
    	...
    	%>		
    			<div class="no_trends"><span>暂无花卉知识信息</span></div>
    	<% 
    		}
    ...
    <%
			...
			%>
				<a href='/FlowerShop/ShowFlowerKnowledgeCl?type=showflowerknowledge&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
     ...
     <%
    		...
    		%>
    			<a href='/FlowerShop/ShowFlowerKnowledgeCl?type=showflowerknowledge&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				...
				<a href='/FlowerShop/ShowFlowerKnowledgeCl?type=showflowerknowledge&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/ShowFlowerKnowledgeCl?type=showflowerknowledge&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
