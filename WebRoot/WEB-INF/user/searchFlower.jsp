...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <li><a href="/FlowerShop/UserMessageCl?type=aftersearch" onClick="return CheckLogin(<%=user %>);">我要留言</a></li>
    ...
    <li><a href="/FlowerShop/UserClServlet?type=showcontact">联系方式</a></li>
  	...
  	<div class="check">感谢您的光临，以下为您查询的结果</div>
  	<div class="user_info">查询得到的花卉<span style="float: right;margin-right: 10px;"><a href="/FlowerShop/index.jsp" style="text-decoration: none;color:#92E587;">返回首页</a></span></div>
  	
  	<!-- 分页显示收藏的花卉，每页显示4条记录 -->
  	<div class="show_restore">
  	
  	<%
   		if(keywordal.size()>0){	//有查询结果
   			for(int i=0;i<keywordal.size();i++){
   				FlowerBean flower = (FlowerBean)keywordal.get(i);
   		%>
  				...
  				<!-- <tr><td colspan="2">收藏时间: &nbsp;<input type="button" value="删除" onclick="return del(<%=flower.getFlowerid() %>)" style="border: 0px;font-size: 18px;background-color:#92E587;"/></td></tr> -->
  				</table>
  				</div>
  				</div>
   		<%
   			...
   			//没有查询结果
   		%>
   			<div class="nokeyword_flower"><span>暂时没有满足条件的花卉,您可以去 <a href="/FlowerShop/UserMessageCl?type=aftersearch" onClick="return CheckLogin(<%=user %>);">留言</a> !</span></div>
   		<% 
  	...
  	<div class="user_info">相同类型的花卉<span style="float: right;margin-right: 10px;"></span></div>
  	...
  	<%
   			...
   			for(int i=0;i<samefloweral.size();i++){
   				FlowerBean flower = (FlowerBean)samefloweral.get(i);
   		%>
   				<div class="restore_flower">
  				<div class="restore_flower_pic"><a href="/FlowerShop/ShowFlowerClServlet?type=showDetail&id=<%=flower.getFlowerid() %>"><img src="images/<%=flower.getPhoto() %>" border="0"/></a></div>
  				<div class="restore_intro">
