...
<html>
  ...
  <!-- 如何防止首页的logo被盗 -->
  	...
  	<%	//此处主要是用于判断用户是点击"立即购买"还是通过购物车来提交订单的
  		//for(int i=0;i<al.size();i++){
  			//FlowerBean flower = (FlowerBean)al.get(i);
  			//System.out.println("展示预订花卉界面中nums="+flower.getReservenums());
  	 %>
  	 <tr><td><%=flowerBean.getFlowerid() %></td><td><%=flowerBean.getFlowername() %></td><td>￥<%=flowerBean.getFlowerprice() %></td><td><%=flowerService.getNumsById(flowerBean) %> <%=flowerBean.getFlowerunit() %></td><td>￥<%=flowerBean.getMarketprice() %></td><td>￥<%=flowerService.getSaveMoney(flowerBean) %></td><td>￥<%=flowerService.getSinglePrice(flowerBean) %></td></tr>
  	 <% //}
  	  %>
  	</table>
  	</div>
  	<form action="/FlowerShop/OrderClServlet?type=reserve" method="post" name="theform">
  	<div class="flower_other_info">
  	<table>
  	<tr><td rowspan="2">给我留言:<input id="extrainfo"  type="text" name="othermessage" value="您的其他需求" onFocus="checkExtraInfo()" /></td><td>付款方式:<input type="radio" name="payedway" value="货到付款"/>货到付款 &nbsp;<input type="radio" name="payedway" value="网银付款"/>网银付款</td></tr>
  	...
  	<div class="total_price_info"><div class="total_price"><font size="4px">订单总价:￥<%=flowerService.getSinglePrice(flowerBean) %><br/>联系人:<%=user.getTruename() %><br/>联系电话:<%=user.getPhone() %></font></div></div>
  	<div class="submit_order"><input onClick="return submitOrder();" type="image" src="images/submitorder.jpg" /></div>
  	</form>
  	<div class="go_back"><input type="image" src="images/gobackindex.jpg" onClick="goback();"/></div>
  	</div>
  <!-- 尾部 -->
   <div class="tail">
