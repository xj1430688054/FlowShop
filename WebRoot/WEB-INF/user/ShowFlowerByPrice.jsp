...
//String flowertype=(String)request.getAttribute("realflowertype");
//ArrayList al = flowerService.getSameTypeFlowersByPage(pageSize,pageNow,flowertype);
//得到共有多少页
//int pageCount = flowerService.getSameFlowerPageCount(pageSize,flowertype);
String lowprice = (String)request.getAttribute("low");
String highprice=(String)request.getAttribute("high");
ArrayList al = flowerService.getFlowersByPriceAndPage(pageSize,pageNow,lowprice,highprice);
int pageCount = flowerService.getFlowerByPricePageCount(pageSize,lowprice,highprice); 
%>
...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <li><a href="/FlowerShop/UserMessageCl?type=aftersearch" onClick="return CheckLogin(<%=user %>);">我要留言</a></li>
    ...
    <li><a href="/FlowerShop/UserClServlet?type=showcontact">联系方式</a></li>
  	...
  	<div class="check1">当前所在>>价格在￥<%=lowprice %> 至 ￥<%=highprice %> 的花卉</div>
  	...
  	%>
  			...
  			<div class="no_store_flower"><div class="no_store_flower_tishi">暂时没有满足该价格区间的花卉!</div></div>
  	<% 
  		}else{
  	 %>
  	...
  	<%
   		//此处开始分页显示记录
   		int start=0;
   		for(int i=0;i<3;i++){	//显示2行，每行两个，可能不够显示
  	...
  	<tr><td>价格:￥<%=flower.getFlowerprice() %>/<%=flower.getFlowerunit() %></td><td>市场价:￥<%=flower.getMarketprice() %>/<%=flower.getFlowerunit() %></td></tr>
  	<tr><td colspan="2">类型:<%=flower.getFlowertype() %> &nbsp;介绍:<%=flower.getFlowerintro() %></td></tr>
  	</table>
  	</div>
  	</div>
  	<%
  	...
  	<div class="restore_flower_fenye"><span>
  	 ...
  	 <%
			...
			%>
				<a href='/FlowerShop/ShowFlowerClServlet?type=searchflower&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
     ...
     %>
  	 ...
  	 <%
    		...
    		%>
    			<a href='/FlowerShop/ShowFlowerClServlet?type=searchflower&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
    		%>
  	 ...
  	 <%
			...
			%>
				...
				<a href='/FlowerShop/ShowFlowerClServlet?type=searchflower&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/ShowFlowerClServlet?type=searchflower&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
