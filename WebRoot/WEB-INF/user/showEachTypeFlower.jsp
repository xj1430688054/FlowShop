...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <li><a href="/FlowerShop/UserMessageCl?type=aftersearch" onClick="return CheckLogin(<%=user %>);">我要留言</a></li>
    ...
    <li><a href="/FlowerShop/UserClServlet?type=showcontact">联系方式</a></li>
  	...
  	<div class="check1">当前所在>>所有<%=flowertype %></div>
  	...
  	%>
  			...
  			<div class="no_store_flower"><div class="no_store_flower_tishi">此类花卉暂时没有!</div></div>
  	<% 
  		}else{
  	 %>
  	...
  	<div class="restore_flower_fenye"><span>
  	 ...
  	 <%
			...
			%>
				<a href='/FlowerShop/ShowFlowerClServlet?type=showflowerbytype&ftype=<%=flowertype %>&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
     ...
     %>
  	 ...
  	 <%
    		...
    		%>
    			<a href='/FlowerShop/ShowFlowerClServlet?type=showflowerbytype&ftype=<%=flowertype %>&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
    		%>
  	 ...
  	 <%
			...
			%>
				...
				<a href='/FlowerShop/ShowFlowerClServlet?type=showflowerbytype&ftype=<%=flowertype %>&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/ShowFlowerClServlet?type=showflowerbytype&ftype=<%=flowertype %>&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
