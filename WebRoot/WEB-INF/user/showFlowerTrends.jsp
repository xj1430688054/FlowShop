...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <li><a href="/FlowerShop/UserMessageCl?type=aftersearch" onClick="return CheckLogin(<%=user %>);">我要留言</a></li>
   ...
   <!-- 右下部分-->
   <div class="show_flower_trends">
   <%
    		if(al.size()!=0){
    %>
    <div class="show_trends">
    			
    		<table border="0">
    <% 
    			for(int m=0;m<al.size();m++){
    			   FlowerTrendsList flowerTrendsList = (FlowerTrendsList)al.get(m);
    %>			
    			<!-- 阅读次数的修改方法：每点击一次超链接，使用Ajax异步修改数据库中的阅读次数 set readtimes=readtimes+1 where id=? -->
    			<tr><td align="left"><a href="/FlowerShop/ShowFlowerTrendsCl?type=nr&no=<%=flowerTrendsList.getId() %>" onclick="addtimes(<%=flowerTrendsList.getId() %>)"><%=flowerTrendsList.getTitle() %></a></td><td align="right">阅读次数(<%=flowerTrendsList.getReadtimes() %>)  <%=flowerTrendsList.getNewstime() %></td></tr>
    ...
    <% 
    	...
    	%>		
    			<div class="no_trends"><span>暂无花卉动态信息</span></div>
    	<% 
    		}
    ...
    <%
			...
			%>
				<a href='/FlowerShop/ShowFlowerTrendsCl?type=showflowernews&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
     ...
     <%
    		...
    		%>
    			<a href='/FlowerShop/ShowFlowerTrendsCl?type=showflowernews&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				...
				<a href='/FlowerShop/ShowFlowerTrendsCl?type=showflowernews&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/ShowFlowerTrendsCl?type=showflowernews&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
