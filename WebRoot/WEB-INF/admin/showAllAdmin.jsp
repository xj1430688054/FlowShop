...
//得到al
ArrayList al = (ArrayList)request.getAttribute("admininfo");
//String spageCount=(String)request.getAttribute("pagecount");
//String spageNow=(String)request.getAttribute("pageNow");
int pageCount = (Integer)request.getAttribute("pagecount");//Integer.parseInt(spageNow);
int pageNow = (Integer)request.getAttribute("pageNow");//Integer.parseInt(spageCount);
%>
  ...
  <head>
	...
	function delAdmin(adminid){
		...
		if(xmlHttpRequest){	//创建成功
				...
				var url="/FlowerShop/AdminClServlet?type=del&mytime="+new Date()+"&adminid="+adminid;
				//打开请求
...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <div class="menu">
  ...
  <ul>
   ...
   <div class="welcome">当前所在>>所有管理员</div>
   <div class="fenye_info">
   <table border="1" cellspacing=0>
   <tr class="tr_css"><td>管理员编号</td><td>管理员名</td><td>权限</td><td>删除管理员</td></tr>
   <%
   
   		for(int i=0;i<al.size();i++){	//分页显示
   			Admin adminInfo= (Admin)al.get(i);
   		%>
   		<tr class="tr_css1"><td><%=adminInfo.getId() %></td>
   		<td><%=adminInfo.getName() %></td>
   		<td><%=adminInfo.getGrade() %></td>
   		<%
   			if(adminInfo.getGrade().equals("超级管理员")){
   		%>
   			<td>------------</td>
   		<% 		
   			}else{
   		%>
   		<td><input type="button" value="删除" onClick="return delAdmin(<%=adminInfo.getId() %>)" style="width:60px;height:25px;background-color:pink;"/></td>
   		<% 
   			}
   		 %>
   ...
   <div class="ye">	
    ...
    <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=admin&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=admin&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=admin&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=admin&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
