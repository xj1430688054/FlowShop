...
<div class="menu">
  ...
  <ul>
   ...
   <div class="welcome">当前所在>>所有交易成功的订单</div>
   <div class="fenye_info"> 
   
    <div class="search_order"><span style="display:block;text-align:left; margin-left:10px;">请输入订单号: <input type="text" id="orderid" name="orderid" style="width:150px;height:20px;border:solid silver;"/>&nbsp;<input type="button" value="查询" onClick="return selectOrder()" style="width:60px;height:28px;background-color:pink;"/><span style="margin-left:60px;"><input type="button" value="返回订单列表" onClick="gotoshowallorder()" style="width:130px;height:28px;background-color:pink;"/></span></span></div>
   ...
   <div class="ye">	
    <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=allfinishorders&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=allfinishorders&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=allfinishorders&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=allfinishorders&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
