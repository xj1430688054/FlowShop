...
//得到al
ArrayList al = (ArrayList)request.getAttribute("usersinfo");
  ...
  <head>
	...
	function delUser(userid){
		...
		if(xmlHttpRequest){	//创建成功
				...
				var url="/FlowerShop/AdminUserCl?type=del&mytime="+new Date()+"&userid="+userid;
				//打开请求
	...
	function chuli(){
		...
		function getDetail(userid){
			...
			//alert("用户编号="+userid);
			//跳转到处理这一请求的控制器
			window.open("/FlowerShop/AdminUserCl?type=userDetail&id="+userid,"_self");
		}
	...
	function selectUser(){
		...
		if(username==""){
			window.alert("请输入要查询的用户名");
			return false;
		}
		//alert("用户名="+username);
		window.open("/FlowerShop/AdminUserCl?type=selectuser&username="+username,"_self");
...
<div class="menu">
  ...
  <ul>
   ...
   <div class="welcome">当前所在>>所有用户</div>
   <div class="fenye_users">
   <div class="search_user"><span>请输入用户名: <input type="text" id="uname" name="username" style="width:150px;height:20px;border:solid silver;"/>&nbsp;<input type="button" value="查询" onClick="return selectUser()" style="width:60px;height:28px;background-color:pink;"/></span></div>
   <table border="1" cellspacing=0>
   <tr class="tr_css"><td>用户编号</td><td>用户名</td><td>性别</td><td>E-mail</td><td>联系电话</td><td>详情</td><td>删除</td></tr>
   <%
   
   		for(int i=0;i<al.size();i++){	//分页显示
   			Users user= (Users)al.get(i);
   		%>
   		<tr class="tr_css1"><td><%=user.getUserid() %></td>
   		<td><%=user.getUsername() %></td>
   		<td><%=user.getSex() %></td>
   		<td><%=user.getEmail() %></td>
   		<td><%=user.getPhone() %></td>
   		<td><input type="button" value="详情" onClick="getDetail(<%=user.getUserid() %>)" style="width:60px;height:25px;background-color:pink;"/></td>
   		<td><input type="button" value="删除" onClick="return delUser(<%=user.getUserid() %>)" style="width:60px;height:25px;background-color:pink;"/></td>
   		</tr>
   ...
   <div class="ye">	
    ...
    <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=users&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=users&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=users&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=users&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
