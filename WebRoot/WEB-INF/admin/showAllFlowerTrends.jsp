...
//得到al
ArrayList al = (ArrayList)request.getAttribute("trends");
...
<html>
  <head>
	...
	function addNews(){
		//添加花卉动态
		window.open("/FlowerShop/AdminFlowerCl?type=addnews","_self");
	}
...
<div class="menu">
  ...
  <ul>
   ...
   <div class="show_allmessage3"> 
    ...
    <div class="welcome1"><span>当前所在>>所有花卉动态</span><span style="margin-left: 620px;"><input type="button" value="添加动态" style="background-color:#E4F0A4;" onclick="addNews()"/></span></div>
   <div class="search_message">
   <span>请输入花卉动态编号: <input type="text" id="message" name="message" style="width:150px;height:20px;border:solid silver;"/>&nbsp;<input type="button" value="查询" onClick="return selectMessage()" style="width:60px;height:28px;background-color:pink;"/></span>
   </div>
   
   <div class="show_messagetitle">
   <table border="0">
   <tr><td class="message_td">编号</td><td>时间</td><td>浏览次数</td><td>删除</td></tr>
   </table>
   </div>
   
   <%
   		for(int i=0;i<al.size();i++){
   			FlowerTrendsList flowerTrendsList = (FlowerTrendsList)al.get(i);
   	%>
   			<div class="show_messagecontent">
   			<div class="nei_rong1"><span><%=flowerTrendsList.getId() %></span></div>
   			<div class="nei_rong1"><span><%=flowerTrendsList.getNewstime() %></span></div>
   			<div class="nei_rong1"><span><%=flowerTrendsList.getReadtimes() %></span></div>
    ...
    <div class="message_con"><span><font color="red">内容:</font><%=flowerTrendsList.getTitle() %></span></div>
    ...
    %>
   		...
   		<%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=flowertrends&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=flowertrends&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=flowertrends&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=flowertrends&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
