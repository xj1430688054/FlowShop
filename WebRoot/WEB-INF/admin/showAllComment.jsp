<%@ page language="java" import="java.util.*,java.text.*,com.gcj.service.admin.*,com.gcj.domain.admin.*,com.gcj.domain.*,com.gcj.service.*" pageEncoding="utf-8"%>
<%
...
//得到al
ArrayList al = (ArrayList)request.getAttribute("comment");
...
<html>
  <head>
	...
	function updComment(orderid,flowerid){	//管理员追加评论
		//alert("订单编号="+orderid);
		//alert("花卉编号="+flowerid);
		//跳转到处理追加的控制器
		window.open("/FlowerShop/AdminCommentCl?type=zhuijia&orderid="+orderid+"&flowerid="+flowerid,"_self");
 ...
 <div class="menu">
  ...
  <ul>
   ...
   <div class="show_allmessage2"> 
    ...
    <div class="welcome1"><span>当前所在>>所有评价</span></div>
   ...
   <table border="0">
   <tr><td class="message_td">订单编号</td><td>花卉编号</td><td>用户名</td><td>评价时间</td><td>追加评论</td></tr>
   </table>
   </div>
   ...
   <%
   	...
   	%>
   			...
   			<%
   				boolean b=commentService.isZhuiJia(comment.getOrderid()+"",comment.getFlowerid()+"");
   				if(b){
   				%>
   					<input type="button" value="追加" onClick="tishi()" style="width:60px;height:25px;background-color:gray;margin-top:7px;"/>	
   				<% 	
   				}else{
   					
   				%>
   					<input type="button" value="追加" onClick="return updComment(<%=comment.getOrderid() %>,<%=comment.getFlowerid() %>)" style="width:60px;height:25px;background-color:pink;margin-top:7px;"/>	
   				<% 
   				}
   			 %>
    ...
    %>
   		...
   		<%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=comment&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=comment&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=comment&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=comment&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
