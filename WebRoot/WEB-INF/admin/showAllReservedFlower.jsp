...
//得到al
ArrayList allreservedfloweral = (ArrayList)request.getAttribute("allreservedfloweral");
...
<div class="menu">
  ...
  <ul>
   ...
   <div class="welcome">当前所在>>所有预订过的花卉</div>
   ...
   <tr class="tr_css"><td>花卉编号</td><td>花卉名称</td><td>预订数量</td><td>预订时间</td><td>花卉单位</td></tr>
   <%
   
   		for(int i=0;i<allreservedfloweral.size();i++){	//分页显示
   			SoldFlower soldFlower = (SoldFlower)allreservedfloweral.get(i);
   ...
   <div class="ye">	
    <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=reservedflowers&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=reservedflowers&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=reservedflowers&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=reservedflowers&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
