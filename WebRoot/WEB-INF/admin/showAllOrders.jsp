...
//得到al
ArrayList orderInfoal = (ArrayList)request.getAttribute("orderinfoal");
...
<html>
  <head>
	...
	function getAllGoingOrder(){		//查询所有交易中的订单
		window.open("/FlowerShop/AdminOrderCl?type=getallgoingorder","_self");
	}
...
<div class="menu">
  ...
  <ul>
   ...
   <div class="welcome">当前所在>>所有订单</div>
   <div class="fenye_info"> 
   
    <div class="search_order"><span style="display:block;text-align:left; margin-left:10px;">请输入订单号: <input type="text" id="orderid" name="orderid" style="width:150px;height:20px;border:solid silver;"/>&nbsp;<input type="button" value="查询" onClick="return selectOrder()" style="width:60px;height:28px;background-color:pink;"/><span style="margin-left:40px;"><input type="button" value="所有交易中的订单" onClick="getAllGoingOrder()" style="width:130px;height:28px;background-color:pink;"/></span><span style="margin-left:10px;"><input type="button" value="所有交易成功的订单" onClick="getAllFinishOrder()" style="width:140px;height:28px;background-color:pink;"/></span></span></div>
   
   <table border="1" cellspacing=0>
   <tr class="tr_css"><td>订单编号</td><td>收花人</td><td>预订时间</td><td>总价</td><td>交易状态</td><td>订单详情</td><td>编辑</td></tr>
   <%
   
   		for(int i=0;i<orderInfoal.size();i++){	//分页显示
   			OrderInfo orderInfo= (OrderInfo)orderInfoal.get(i);
   			String state = orderInfo.getTradestate();
   		%>
   		<tr class="tr_css1"><td><%=orderInfo.getId() %></td>
   		<td><%=orderInfo.getTruename() %></td>
   		<td><%=orderInfo.getOrderDate() %></td>
   		<td>￥<%=orderInfo.getTotalPrice() %></td>
   		<td><%=orderInfo.getTradestate() %></td>
   		<td><input type="button" value="查看" onClick="return getOrderDetail(<%=orderInfo.getId() %>)" style="width:60px;height:25px;background-color:pink;"/></td>
   		<td>
   		<%
   		...
   		%>		
   				<input type="button" value="编辑" onClick="return updOrder(<%=orderInfo.getId() %>)" style="width:60px;height:25px;background-color:pink;"/>	
   		<% 
   			}
   		 %>
   ...
   <div class="ye">	
    <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=orders&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=orders&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=orders&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=orders&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
