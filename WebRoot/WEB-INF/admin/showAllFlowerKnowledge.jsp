...
//得到al
ArrayList al = (ArrayList)request.getAttribute("knowledges");
...
<html>
  <head>
	...
	function selectMessage(){	//查询花卉动态
		...
		window.open("/FlowerShop/AdminFlowerCl?type=selectflowerknowledge&no="+message,"_self");
	}
	...
	function del(knowledgeid){
	  if(window.confirm("您真要删除吗?")){
		...
		if(xmlHttpRequest){	//创建成功
				...
				var url="/FlowerShop/AdminFlowerCl?type=delflowerknowledge&mytime="+new Date()+"&no="+knowledgeid;
				//打开请求
	...
	function addKnowledge(){
		//添加花卉动态
		window.open("/FlowerShop/AdminFlowerCl?type=addKnowledge","_self");
	}
...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
 ...
 <div class="menu">
  ...
  <ul>
   ...
   <div class="show_allmessage3"> 
   
    <div class="welcome1"><span>当前所在>>所有花卉知识</span><span style="margin-left: 620px;"><input type="button" value="添加知识" style="background-color:#E4F0A4;" onclick="addKnowledge()"/></span></div>
   <div class="search_message">
   <span>请输入花卉知识编号: <input type="text" id="message" name="message" style="width:150px;height:20px;border:solid silver;"/>&nbsp;<input type="button" value="查询" onClick="return selectMessage()" style="width:60px;height:28px;background-color:pink;"/></span>
   </div>
   ...
   <%
   		for(int i=0;i<al.size();i++){
   			FlowerKnowledge flowerKnowledge = (FlowerKnowledge)al.get(i);
   	%>
   			<div class="show_messagecontent">
   			<div class="nei_rong1"><span><%=flowerKnowledge.getId() %></span></div>
   			<!--  <div class="nei_rong"><span></span></div>-->
   			<div class="nei_rong1"><span><%=flowerKnowledge.getPublishtime() %></span></div>
   			<div class="nei_rong1"><span><%=flowerKnowledge.getReadtimes() %></span></div>
   			<div class="nei_rong1"><span>
   			
             <input type="button" value="删除"  style="width:60px;height:25px;background-color:#FFC0CB;" onClick="return del(<%=flowerKnowledge.getId() %>)"/>
   			
   			</span></div>
   			</div>
   			<div class="message_con"><span><font color="red">标题:</font><%=flowerKnowledge.getTitle() %></span></div>
   			 <div class="message_con"><span><font color="red">内容:</font><%=flowerKnowledge.getContent() %></span></div>
    ...
    %>
   		...
   		<%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=flowerknowledge&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=flowerknowledge&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=flowerknowledge&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=flowerknowledge&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
