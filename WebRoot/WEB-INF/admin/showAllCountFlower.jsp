...
//得到al
ArrayList allsamefloweral = (ArrayList)request.getAttribute("allsamefloweral");
...
<html>
  <head>
	...
	function selectOrder(){	//查询订单
		...
		window.open("/FlowerShop/AdminFlowerCl?type=selectflowernum&no="+id,"_self");
	}
	...
	function goback(){
		window.open("/FlowerShop/AdminFlowerCl?type=reservedflower","_self");
	}
	</script>
...
<table width="78%" height="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <ul>
   ...
   <div class="welcome">当前所在>>相同花卉统计</div>
   <div class="fenye_info"> 
   
   	<%
  				if(allsamefloweral.size()<=0){
  		%>			
    ...
    <div class="search_order"><span style="display:block;text-align:left; margin-left:530px;"><!--  请输入预订总量: <input type="text" id="orderid" name="orderid" style="width:150px;height:20px;border:solid silver;"/>&nbsp;<input type="button" value="按预定数量查询" onClick="return selectOrder()" style="width:130px;height:28px;background-color:pink;"/>--><span style="margin-left: 20px;"><input type="button" value="返回" onClick="goback()" style="width:140px;height:28px;background-color:pink;"/></span></span></div>
   
   <table border="1" cellspacing=0>
   <tr class="tr_css"><td>花卉编号</td><td>花卉名称</td><td>预订数量</td><td>花卉单位</td></tr>
   <%
   
   		for(int i=0;i<allsamefloweral.size();i++){	//分页显示
   			SoldFlower soldFlower = (SoldFlower)allsamefloweral.get(i);
   ...
   <div class="ye">	
    <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=getallsameflower&pageNow=<%=pageNow-1 %>'>上一页</a>
			<% 
			}
    ...
    <%
    		...
    		%>
    			<a href='/FlowerShop/AdminFenyeCl?type=getallsameflower&pageNow=<%=i %>'>[<%=i %>]</a>
    		<%
    		}
     ...
     <%
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=getallsameflower&pageNow=<%=pageNow+1 %>'>下一页</a>
			<% 
			}
			//最后一页
			...
			%>
				<a href='/FlowerShop/AdminFenyeCl?type=getallsameflower&pageNow=<%=pageCount %>'>最后一页</a>
			<% 
			}
			%>
			<% 
